...
class TestSelectableMenu:
    ...
    def test_initialization(self, mock_selectable_menu):
        ...
        assert (
            menu.title == "Test Menu: Option 1"
        ), "Menu title should include selected option"
        assert len(menu._menu_items) == 3, "Menu should have 3 options"
        for item in menu._menu_items:
            assert isinstance(
                item, MenuItem
            ), "Sub-menu items should be MenuItem instances"
            assert (
                item.callback == menu._option_selected
            ), "Sub-menu items should have correct callback"
            assert item.state == (
                1 if item.title == "Option 1" else 0
            ), "Selected item should have state 1, others should have state 0"
    ...
    def test_option_selection(self, mock_selectable_menu):
        menu, mock_cb = mock_selectable_menu()
        assert (
            menu.get_selected() == "Option 1"
        ), "Initially selected option should be 'Option 1'"
        # Simulate selecting "Option 2"
        option_2_item = next(
            item for item in menu._menu_items if item.title == "Option 2"
        )
        menu.set_selected("Option 2")
        assert option_2_item.state == 1, "Selected item should have state 1"
        assert (
            menu.title == "Test Menu: Option 2"
        ), "Menu title should update to selected option"
        for item in menu._menu_items:
    ...
    def test_no_options(self, mock_selectable_menu):
        ...
        menu.set_selected(
            "Option 1"
        )  # Should not raise an error even though there are no options
    ...
    def test_set_selected_with_invalid_option(self, mock_selectable_menu):
        ...
        menu.set_selected(
            "Invalid Option"
        )  # Should not raise an error, but should not change selection
        assert (
            menu.get_selected() is None
        ), "Invalid selected option should result in no selection"
        assert (
            menu.title == "Test Menu"
        ), "Menu title should not include selected option if selection is invalid"
